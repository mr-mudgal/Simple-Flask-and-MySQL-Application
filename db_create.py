...
conn = mysql.connector.connect(host=os.getenv('HOSTNAME'), user=os.getenv('USER'), password=os.getenv('PASSWORD'))
...
try:
    ...
    try:
        ...
        conn = mysql.connector.connect(
            host="localhost", user="root", password="Mac#Mysql.1", database="users"
        )
